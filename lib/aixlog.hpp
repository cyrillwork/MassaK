...
static std::map<int8_t, std::string> gLogsMap =
{
    { INFO, "DataCapture" }
};
...
static Severity to_severity(std::string severity, Severity def = Severity::DataCapture)
{
    //std::transform(severity.begin(), severity.end(), severity.begin(), [](unsigned char c) { return static_cast<char>(std::tolower(c)); });    
    for(const auto& it: gLogsMap) {
        if(it.second == severity) {
            return static_cast<Severity>(it.first);
        }
    }
    return def;
}
...
{
    auto it = gLogsMap.find( static_cast<int8_t>(logSeverity) );
    if(it != gLogsMap.end()) {
        return it->second;
    }
...
/*
    switch (logSeverity)
    {
        case Severity::DataCapture:
            return "DataCapture";
        case Severity::TITO:
            return "TITO";
        case Severity::Promotional:
            return "Promotional";
...
struct Metadata
{
    Metadata() : severity(Severity::DataCapture), tag(nullptr), function(nullptr), timestamp(nullptr)
    {
    }
...
protected:
    ...
    int sync() override
    {
        const size_t MAX_STR = 70;
        std::lock_guard<std::recursive_mutex> lock(mutex_);

        if (!get_stream().str().empty())
        {
            auto _str = get_stream().str();
            //do not std::cout << _str << std::endl;
            if (do_log_)
            {
                for (const auto& sink : log_sinks_)
                {
                    if (sink->filter.match(metadata_)) {
                        if(_str.size() > MAX_STR) {
                            size_t _pos = 0;
                            ...
                            while(_size > 0) {
                                ...
                                auto _temp_str = _str.substr(_pos, _len);
                                sink->log(metadata_, _temp_str);
                                _pos  += _len;
                                _size -= _len;
                            }
                        } else {
                            sink->log(metadata_, _str);
                        }
...
{
    SinkFile(const Filter& filter, const std::string& filename, const std::string& format = "%Y-%m-%d %H-%M-%S.#ms"/*"%Y-%m-%d %H-%M-%S.#ms [#severity]"*/ /*"%Y-%m-%d %H-%M-%S.#ms [#severity] (#tag_func)"*/)
        : SinkFormat(filter, format)
    {
        _filename = filename;
        ofs.open(_filename.c_str(), std::ofstream::out | /*std::ofstream::trunc*/std::ofstream::app);
    }
    ...
    void rotateLog() {
        ofs.close();
        std::string new_file_path = _filename + ".old";
        std::rename(_filename.c_str(), new_file_path.c_str());
        ofs.open(_filename.c_str(),  std::ofstream::out | std::ofstream::trunc);
    }

    bool isOverFlow(){
        ofs.seekp(0, std::ios::end);
        size_t size = ofs.tellp();
        return (size >= MAX_SIZE_LOG_FILE);
    }
    void log(const Metadata& metadata, const std::string& message) override
    {
